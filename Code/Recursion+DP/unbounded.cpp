...
int dpTbl[8+1][4+1]; // w_remaining rows and n columns, but we want +1 on both rows and cols.(title naming for first row and col)
...
int knapsack(int* weights, int* vals, int w_remaining, int n){
    ...
    //RECURSE: 
     if(w_remaining - weights[n-1] >= 0){
        //PICKING IT: 
        opt_Val_1 = vals[n-1] + knapsack(weights, vals,  w_remaining - weights[n-1], n);
        // NOT PICKING IT:
        ...
        // n-1 bc we NEVER can choose this
        dpTbl[w_remaining][n] = knapsack(weights, vals, w_remaining, n-1);  // 1. Insead of returning, store the value in dpTbl
        return dpTbl[w_remaining][n]; // 2. Return what you just stored in dpTbl form
    }
...
int main(){
    ...
    //make a weights arr: 
    int weights_Arr[] = {1,3,4,5};
    //make a values array:
    int values_Arr[] = {10,40,50,70}; 
    //make a capacity: 
    const int capacity = 8; 
    int answer = knapsack(weights_Arr, values_Arr, capacity, 4); 
